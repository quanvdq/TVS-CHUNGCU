...
{
    public partial class rptNotify : DataDynamics.ActiveReports.ActiveReport3
    {        
        ...
        public rptNotify()
        {
            ...
            try
            {
                ...
                if (sMoneyWater != 0)
                    this.LblMoneyWater.Text = "4: Phí tiền nước :  Chỉ số đầu " + string.Format("{0:###,0}", double.Parse(srptTable.Rows[0]["NumberBeginWater"].ToString())) + " cuối " + string.Format("{0:###,0}", double.Parse(srptTable.Rows[0]["NumberEndWater"].ToString())) + ". Tiền: " + string.Format("{0:###,0}", sMoneyWater) + " đồng";
                else
                    this.LblMoneyWater.Text = "";
                if (sMoneyElectric != 0)
                    this.LblMoneyElectric.Text = "5: Phí tiền điện :  Chỉ số đầu " + string.Format("{0:###,0}", double.Parse(srptTable.Rows[0]["NumberBeginElectric"].ToString())) + " cuối " + string.Format("{0:###,0}", double.Parse(srptTable.Rows[0]["NumberEndElectric"].ToString())) + ". Tiền: " + string.Format("{0:###,0}", sMoneyElectric) + " đồng";
                else
                    this.LblMoneyElectric.Text = "";
        ...
        private void rpt_ReportStart(object sender, EventArgs e)
        {
            ...
            rptWater.srptID = srptID;
            this.subWater.Report = new rptWater();
            rptElectric.srptID = srptID;
            this.subElectric.Report = new rptElectric();
        }
    }
